    ...
    <>
      ...
      <header className="sticky top-0 z-30 header-shadow relative bg-[#FEF9EE]">
        <div className="relative container mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <nav className="hidden md:flex space-x-4 items-center text-[#FF5722]" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
            <Link to="/services" onClick={handleNavLinkClick} className={`px-3 py-2 rounded-md hover:bg-[#FF5722]/10 transition-colors duration-300 tracking-wider text-lg`}>SERVICES</Link>
            <Link to="/rentals" onClick={handleNavLinkClick} className={`px-3 py-2 rounded-md hover:bg-[#FF5722]/10 transition-colors duration-300 tracking-wider text-lg`}>OUR GEAR</Link>
            <Link to="/careers" onClick={handleNavLinkClick} className={`px-3 py-2 rounded-md hover:bg-[#FF5722]/10 transition-colors duration-300 tracking-wider text-lg`}>CAREERS</Link>

            {/* --- "ABOUT US" DROPDOWN REMOVED --- */}

            <button
              onClick={handleMyPageClick}
              className="ml-4 border border-[#FF5722] text-[#FF5722] hover:bg-[#FF5722] hover:text-white font-bold py-2 px-4 rounded-lg text-base transition-colors"
            >
              MY PAGE
            </button>
          </nav>
          <button ref={toggleButtonRef} onClick={() => setMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden p-2 -mr-2 rounded-full">
            <div className="relative w-6 h-6">{isMobileMenuOpen ? <X className="text-[#FF5722]" /> : <Menu className="text-[#FF5722]" />}</div>
          </button>
        </div>
      </header>
        ...
        <div id="mobile-menu-panel" ref={menuRef} role="dialog" aria-modal="true" className={`absolute top-0 left-0 w-full transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"} ${isMobileMenuOpen ? "menu-open" : ""} bg-white border-b border-gray-200 shadow-2xl`}>
          <div className="safe-padding-top">
            <div className="container mx-auto px-6 pt-20 pb-8">
              <nav className="flex flex-col text-[#FF5722]" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
                {navItems.map((item, index) => {
                  ...
                  return (
                    <div key={item.name} className="menu-item border-b border-gray-900/10" style={{ transitionDelay: prefersReducedMotion ? "0ms" : `${100 + index * 40}ms` }}>
                      <Link to={item.href!} onClick={handleNavLinkClick} className={`relative block py-4 px-3 text-xl tracking-wider rounded-md hover:bg-[#FF5722]/10 ${isActive ? "font-bold" : ""}`}>
                        {isActive && (<span className="absolute left-0 top-1/2 -translate-y-1/2 h-6 w-1 bg-[#FF5722]/60 rounded-r-full"></span>)}
                        {item.name}
                      </Link>
                    </div>
                  );
                ...
                <button
                  onClick={handleMyPageClick}
                  className="block mt-4 w-full border border-[#FF5722] text-[#FF5722] hover:bg-[#FF5722] hover:text-white font-bold py-3 px-4 rounded-lg text-center transition-colors"
                >
                  MY PAGE
                </button>
              </nav>
